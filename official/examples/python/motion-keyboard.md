...
_**注意:运行本示例前需要先使用手柄，将机器人切入稳定站立模式。（位控站立/走跑模式时按` R2`+`X`, 其他模式详见[模式流转图](/official/quickstart/run_example#fig-routing-to-stand-default)进行操作），然后在机器人的终端界面使用：`aima em stop-app rc`关闭遥控器，防止通道占用。**_
